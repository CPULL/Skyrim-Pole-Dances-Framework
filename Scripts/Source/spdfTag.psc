Scriptname spdfTag extends ReferenceAlias
...
string Function _tryToParseTags(string tagCode, string[] validTags, string[] bodyParts, spdfRegistry reg) global
	...
	while i
		...
		elseIf theTag=="Strip" || theTag=="StripAn"
			...
			while j
				...
				endIf
				if slots[j]!="All" && bodyParts.find(slots[j])==-1
...
bool Function _init(string tagCode, string[] validTags, spdfPoleDances spd)
	tags = new string[1]
	...
	while i
		...
		elseIf theTag=="Strip"
			...
			while j
				...
				endIf
				if slots[j]!="All" && spdF.registry.bodyParts.find(slots[j])==-1
